/*------------------------------------------------
File Name: List.java
Desc: Generic List ADT using <T> types
Instructions: None
...
public class List<T> {
  ...
  T back() throws RuntimeException {
    if (length() <= 0)
  ...
  void insertBefore(T data) throws RuntimeException {
    if (length() <= 0)
  ...
  // Pre: length()>0, index()>=0
  void insertAfter(T data) throws RuntimeException {
    if (length() <= 0)
  ...
  // Hlper to move to a certian node
  void moveTo(int index){
    ...
    for(int i=1; i<=index; i++) {
      moveNext();
      //System.out.println("--Moved to " + index() + "/"+length());
    }
